    ...
    def toSTIX21(self):
        ...
        for x in self.property_path:
            ...
            if current_cyber_observable_type == "file":
                if (
                    x.property_name == "is_encrypted"
                    or x.property_name == "encryption_algorithm"
                    or x.property_name == "decryption_key"
                ):
                    print(
                        "Expression contains the property "
                        + x.property_name
                        + ", for a file, which is not in STIX 2.1"
                    )
                elif (
                    x.property_name == "archive-ext"
                    or x.property_name == "raster-image-ext"
                ):
                ...
                if x.property_name == "version":
                    print(
                        "Expression contains the property version, for a file.archive-ext, which is not in STIX 2.1"
                    )
                ...
                if x.property_name == "name" or x.property_name == "arguments":
                    print(
                        "Expression contains the property "
                        + x.property_name
                        + ", for a process, which is not in STIX 2.1"
                    )
...
class ComparisonExpressionForElevator(_ComparisonExpression):
    ...
    def __init__(self, operator, lhs, rhs, negated=False):
        ...
        if operator == "=" and isinstance(rhs, stix2.ListConstant):
            warn(
                "apply_condition assumed to be 'ANY' in %s",
                721,
    ...
    def replace_placeholder_with_idref_pattern(self, idref):
        if isinstance(self.rhs, IdrefPlaceHolder):
            change_made, pattern = self.rhs.replace_placeholder_with_idref_pattern(
                idref
            )
            if change_made:
                if hasattr(self.lhs, "collapsed") and self.lhs.collapsed:
                    return True, ComparisonExpressionForElevator(
                        pattern.operator, self.lhs, pattern.rhs
                    )
                else:
...
class EqualityComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(EqualityComparisonExpressionForElevator, self).__init__(
            "=", lhs, rhs, negated
        )
...
class MatchesComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(MatchesComparisonExpressionForElevator, self).__init__(
            "MATCHES", lhs, rhs, negated
        )


class GreaterThanComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(GreaterThanComparisonExpressionForElevator, self).__init__(
            ">", lhs, rhs, negated
        )
...
class LessThanComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(LessThanComparisonExpressionForElevator, self).__init__(
            "<", lhs, rhs, negated
        )
...
class GreaterThanEqualComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(GreaterThanEqualComparisonExpressionForElevator, self).__init__(
            ">=", lhs, rhs, negated
        )
...
class LessThanEqualComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(LessThanEqualComparisonExpressionForElevator, self).__init__(
            "<=", lhs, rhs, negated
        )
...
class LikeComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(LikeComparisonExpressionForElevator, self).__init__(
...
class IsSubsetComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(IsSubsetComparisonExpressionForElevator, self).__init__(
            "ISSUBSET", lhs, rhs, negated
        )


class IsSupersetComparisonExpressionForElevator(ComparisonExpressionForElevator):
    def __init__(self, lhs, rhs, negated=False):
        super(IsSupersetComparisonExpressionForElevator, self).__init__(
            "ISSUPERSET", lhs, rhs, negated
        )
...
class BooleanExpressionForElevator(_BooleanExpression):
    ...
    def replace_placeholder_with_idref_pattern(self, idref):
        ...
        change_made = False
        for args in self.operands:
            change_made_this_time, new_operand = (
                args.replace_placeholder_with_idref_pattern(idref)
            )
    ...
    def partition_according_to_object_path(self):
        ...
        for x in subexpressions:
            if len(x) == 1:
                ...
                results.append(
                    create_boolean_expression(self.operator, x, use_parens=False)
                )
        if len(results) == 1:
            return results[0]
        else:
...
class FollowedByObservationExpressionForElevator(
    ...
    def __init__(self, operands):
        super(FollowedByObservationExpressionForElevator, self).__init__(
            "FOLLOWEDBY", operands
        )
...
class QualifiedObservationExpressionForElevator(QualifiedObservationExpression):
    ...
    def __init__(self, observation_expression, qualifier):
        super(QualifiedObservationExpressionForElevator, self).__init__(
...
class ParentheticalExpressionForElevator(stix2.ParentheticalExpression):
    ...
    def replace_placeholder_with_idref_pattern(self, idref):
        change_made, new_expression = (
            self.expression.replace_placeholder_with_idref_pattern(idref)
        )
...
_ADDRESS_NAME_MAPPING = {
    Address.CAT_IPV4: "ipv4-addr",
...
def convert_cybox_class_name_to_object_path_root_name(instance):
    ...
    if class_name in _CLASS_NAME_MAPPING:
        ...
        error(
            "Cannot convert CybOX 2.x class name %s to an object_path_root_name",
            813,
            class_name,
        )
        return None
...
def convert_condition(condition):
    if condition is None:
        warn(
            "No condition given for term in %s - assume '='",
            714,
            identifying_info(get_dynamic_variable("current_observable")),
        )
        return "="
    for cond, op in _CONDITION_OPERATOR_MAP.items():
        if cond.lower() == condition.lower():
            ...
            return op
    warn(
        "Unknown condition given in %s - marked as 'INVALID_CONDITION'",
        628,
        identifying_info(get_dynamic_variable("current_observable")),
    )
...
def create_term_with_range(lhs, condition, rhs, negated=False):
    ...
    if not isinstance(rhs, stix2.ListConstant) or len(rhs.value) != 2:
        ...
        if condition == "InclusiveBetween":
            ...
            lower_bound = ComparisonExpressionForElevator(
                process_comparison_negation(">=", negated), lhs, rhs.value[0]
            )
            upper_bound = ComparisonExpressionForElevator(
                process_comparison_negation("<=", negated), lhs, rhs.value[1]
            )
        ...
        return create_boolean_expression(
            process_boolean_negation("AND", negated), [lower_bound, upper_bound]
        )
...
def create_term(lhs, condition, rhs, negated=False):
    if condition == "StartsWith" or condition == "EndsWith":
        ...
        return ComparisonExpressionForElevator(
            convert_condition(condition), lhs, rhs, negated
        )
...
def make_constant(obj):
    ...
    elif isinstance(obj, datetime.datetime) or isinstance(
        obj, stixmarx.api.types.MarkableDateTime
    ):
...
def convert_custom_properties(cps, object_type_name, use_custom_prefix=True):
    ...
    for cp in cps.property_:
        if not re.match("[a-z0-9_]+", cp.name):
            warn(
                "The custom property name %s does not adhere to the specification rules",
                617,
                cp.name,
            )
            if " " in cp.name:
        ...
        expressions.append(
            create_term(
                object_type_name + ":" + custom_name,
                cp.condition,
                make_constant(cp.value),
            )
        )
    ...
    if isinstance(account, UnixUserAccount):
        ...
        expressions.append(
            create_term(
                ...
                "Equals",
                stix2.StringConstant(
                    "windows-domain" if account.domain else "windows-local"
                ),
            )
        )
    if expressions:
...
def convert_artifact_to_pattern(art):
    ...
    if art.content_type:
        expressions.append(
            create_term(
                "artifact:mime_type", art.content_type.condition, art.content_type
            )
        )
    if art.raw_artifact:
        expressions.append(
...
def convert_unix_user_to_pattern(account):
    expressions = []
    expressions.append(
        create_term("user-account:account_type", "Equals", stix2.StringConstant("unix"))
    )
...
def convert_address_to_pattern(add):
    ...
    if add.category == add.CAT_IPV4:
        return create_term(
            "ipv4-addr:value", cond, make_constant(add.address_value.value)
        )
    elif add.category == add.CAT_IPV6:
        return create_term(
            "ipv6-addr:value", cond, make_constant(add.address_value.value)
        )
    elif add.category == add.CAT_MAC:
        return create_term(
            "mac-addr:value", cond, make_constant(add.address_value.value)
        )
    elif add.category == add.CAT_EMAIL:
        return create_term(
            "email-addr:value", cond, make_constant(add.address_value.value)
        )
    else:
...
def convert_as_to_pattern(a_s):
    ...
    if a_s.number:
        expressions.append(
            add_comparison_expression(a_s.number, "autonomous-system:number")
        )
    if a_s.name:
        expressions.append(
            add_comparison_expression(a_s.name, "autonomous-system:name")
        )
    if a_s.regional_internet_registry:
        expressions.append(
            add_comparison_expression(
                a_s.regional_internet_registry, "autonomous-system:rir"
            )
        )
    if expressions:
        return create_boolean_expression("AND", expressions)
...
_EMAIL_ADDITIONAL_HEADERS_PROPERTIES = [
    ...
    [
        "email-message:additional_header_fields.X-Originating-IP",
        ["x_originating_ip", "address_value"],
    ],
    ...
    if len(prop_list) == 1:
        ...
        if hasattr(obj, canonical_prop1x_name):
            if multi_valued_property(prop_1x):
                ...
                return add_comparison_expression(
                    getattr(obj, canonical_prop1x_name), object_path
                )
    else:
        ...
        if hasattr(obj, canonical_prop1x_name):
            if multi_valued_property(prop_1x):
                ...
                if values:
                    for c in values:
                        term = create_terms_from_prop_list(
                            rest_of_prop_list, c, object_path
                        )
                        if term:
                            prop_exprs.append(term)
                # return " OR ".join(prop_exprs)
                ...
                return create_terms_from_prop_list(
                    rest_of_prop_list, getattr(obj, canonical_prop1x_name), object_path
                )
...
def handle_message_id_property(head):
    if head.message_id:
        if get_option_value("spec_version") == "2.1":
            ...
            lhs = generate_lhs_for_missing_property(
                "email-message:", None, "message_id", "email-message"
            )
        if lhs:
            return create_term(
                lhs,
                head.message_id.condition,
                stix2.StringConstant(str(head.message_id)),
            )
...
def convert_email_message_to_pattern(mess):
    ...
    if mess.header is not None:
        expressions.append(
            convert_email_header_to_pattern(mess.header, _EMAIL_HEADER_PROPERTIES)
        )
        message_id_term = handle_message_id_property(mess.header)
        if message_id_term:
            expressions.append(message_id_term)
        add_headers = convert_email_header_to_pattern(
            mess.header, _EMAIL_ADDITIONAL_HEADERS_PROPERTIES
        )
            ...
            if isinstance(new_pattern, IdrefPlaceHolder):
                expressions.append(
                    ComparisonExpressionForElevator(
                        "=", "email-message:body_multipart[*].body_raw_ref", new_pattern
                    )
                )
            else:
                expressions.append(
                    new_pattern.collapse_reference(
                        ObjectPathForElevator.make_object_path(
                            "email-message:body_multipart[*].body_raw_ref"
                        )
                    )
                )
    ...
    if mess.links is not None:
        if get_option_value("spec_version") == "2.1":
            lhs = generate_lhs_for_missing_property(
                "email-message:", None, "link_refs[*].value", "email-message"
            )
            if lhs:
            ...
            warn(
                "Observed Data objects cannot refer to other external objects (in STIX 2.0): %s in %s",
                434,
                "links",
                "email-message",
            )
...
_PE_FILE_HEADER_PROPERTIES = [
    ...
    [
        ...
        "file:extensions.'windows-pebinary-ext'.pointer_to_symbol_table",
    ],
    ["number_of_symbols", "file:extensions.'windows-pebinary-ext'.number_of_symbols"],
    [
        "size_of_optional_header",
        "file:extensions.'windows-pebinary-ext'.size_of_optional_header",
    ],
    ["characteristics", "file:extensions.'windows-pebinary-ext'.characteristics_hex"],
]


_PE_SECTION_HEADER_PROPERTIES = [
    ["name", "file:extensions.'windows-pebinary-ext'.sections[*].name"],
        ...
        if file_header:
            ...
            for prop_spec in _PE_FILE_HEADER_PROPERTIES:
                ...
                if hasattr(file_header, prop_1x) and getattr(file_header, prop_1x):
                    term = add_comparison_expression(
                        getattr(file_header, prop_1x), object_path
                    )
                    if term:
            ...
            if file_header_expressions:
                expressions.append(
                    create_boolean_expression("AND", file_header_expressions)
                )
        if f.headers.optional_header:
            warn(
                "file:extensions:'windows-pebinary-ext':optional_header is not implemented yet",
                807,
            )
        ...
        for s in sections:
            ...
            if s.section_header:
                for prop_spec in _PE_SECTION_HEADER_PROPERTIES:
                    prop_1x = prop_spec[0]
                    object_path = prop_spec[1]
                    if hasattr(s.section_header, prop_1x) and getattr(
                        s.section_header, prop_1x
                    ):
                        term = add_comparison_expression(
                            getattr(s.section_header, prop_1x), object_path
                        )
                        if term:
                ...
                if s.entropy.min:
                    if check_for_missing_policy("use-custom-properties"):
                        section_expressions.append(
                            create_term(
                                "file:extensions.'windows-pebinary-ext'.sections[*]."
            ...
            if section_expressions:
                sections_expressions.append(
                    create_boolean_expression("AND", section_expressions)
                )
        if sections_expressions:
            expressions.append(create_boolean_expression("AND", sections_expressions))
    if f.exports:
        warn(
            "The exports property of WinExecutableFileObj is not part of STIX 2.x", 418
        )
        lhs = generate_lhs_for_missing_property(
            "file:", "windows-pebinary-ext", "exports[*]", "file"
        )
        if lhs:
            ...
            if not check_for_missing_policy("ignore"):
                expressions.append(
                    UnconvertedTerm("WinExecutableFileObj.exports", "file")
                )
    if f.imports:
        warn(
            "The imports property of WinExecutableFileObj is not part of STIX 2.x", 418
        )
        lhs = generate_lhs_for_missing_property(
            "file:", "windows-pebinary-ext", "imports[*]", "file"
        )
        if lhs:
            ...
            for i in f.imports:
                if hasattr(i, "imported_functions"):
                    file_name = (
                        i.file_name + ":"
                        if hasattr(i, "file_name") and i.file_name
                        else ""
                    )
                    for imported_func in i.imported_functions:
                        import_expressions.append(
                            create_term(
                                ...
                                stix2.StringConstant(
                                    file_name + imported_func.function_name.value
                                ),
                            )
                        )
            ...
            if not check_for_missing_policy("ignore"):
                expressions.append(
                    UnconvertedTerm("WinExecutableFileObj.imports", "file")
                )
    if expressions:
        return create_boolean_expression("AND", expressions)


_ARCHIVE_FILE_PROPERTIES_2_0 = [
    ["comment", "file:extensions.'archive-ext'.comment"],
...
def convert_archive_file_to_pattern(f):
    ...
    if f.archived_file:
        ...
        for a_f in f.archived_file:
            ...
            archived_file_expressions.append(
                terms.collapse_reference(
                    ObjectPathForElevator.make_object_path(
                        "file:extensions.archive-ext.contains_refs[*]"
                    )
                )
            )
        if archived_file_expressions:
            and_expressions.append(
                create_boolean_expression("AND", archived_file_expressions)
            )
    if and_expressions:
...
def convert_pdf_file_to_pattern(f):
    ...
    if f.version:
        and_expressions.append(
            create_term(
                "file:extensions.'pdf-ext'.version",
                f.version.condition,
    ...
    if f.metadata:
        if f.metadata.optimized:
            and_expressions.append(
                create_term(
                    "file:extensions.'pdf-ext'.is_optimized",
        ...
        if f.metadata.document_information_dictionary:
            ...
            for key in PDF_DOC_INFO:
                ...
                if value:
                    and_expressions.append(
                        create_term(
                            "file:extensions.'pdf-ext'.document_info_dict."
                            + PDF_DOC_INFO_DICT[key],
                            value.condition,
    ...
    if f.trailers:
        ...
        for t in f.trailers:
            if t.id_:
                for file_id in t.id_.id_string:
                    if count == 2:
                        warn(
                            "Only two pdfids are allowed for %s, dropping %s",
                            505,
                            f.id_,
                            file_id,
                        )
        ...
        if len(file_ids) == 2:
            and_expressions.append(
                create_term(
                    "file:extensions.'pdf-ext'.pdfid0",
            ...
            and_expressions.append(
                create_term(
                    "file:extensions.'pdf-ext'.pdfid1",
...
def convert_hashes_to_pattern(hashes):
    ...
    for h in hashes:
        ...
        hash_expressions.append(
            create_term("file:hashes" + "." + hash_type, hash_value.condition, hc)
        )
    if hash_expressions:
...
def convert_file_name_and_file_extension(file_name, file_extension):
    if (
        file_extension
        and file_extension.value
        and is_equal_condition(file_name.condition)
        and is_equal_condition(file_extension.condition)
        and file_name.value.endswith(file_extension.value)
    ):
        return create_term(
            "file:name", file_name.condition, make_constant(file_name.value)
        )
    elif (
        file_name.condition == "StartsWith"
        and file_extension
        and file_extension.value
        and is_equal_condition(file_extension.condition)
    ):
        return ComparisonExpressionForElevator(
            "MATCHES",
            "file:name",
            make_constant("^" + file_name.value + "*." + file_extension.value + "$"),
        )
    elif (
        file_name.condition == "Contains"
        and file_extension
        and file_extension.value
        and is_equal_condition(file_extension.condition)
    ):
...
def convert_file_name_and_path_to_pattern(f):
    ...
    if f.file_name and f.file_extension and f.file_extension.value:
        file_name_path_expressions.append(
            convert_file_name_and_file_extension(f.file_name, f.file_extension)
        )
    elif f.file_name:
        file_name_path_expressions.append(
            create_term(
                "file:name", f.file_name.condition, make_constant(f.file_name.value)
            )
        )
    if f.file_path and f.file_path.value:
        ...
        if index == -1:
            ...
            if (
                not f.file_path.value.endswith("/")
                and not f.file_path.value.endswith("\\")
                and not f.file_name
            ):
                file_name_path_expressions.append(
                    create_term(
                ...
                path_string_constant = make_constant(
                    (
                ...
                path_string_constant = make_constant(
                    ((f.device_path.value if f.device_path else "") + f.file_path.value)
                )
                if path_string_constant == "":
                    warn("File path directory is empty %s", 633, f.file_path.value)
                file_name_path_expressions.append(
                ...
                file_name_path_expressions.append(
                    create_term(
                        "directory:path", f.file_path.condition, path_string_constant
                    )
                )
    if f.full_path:
...
def convert_file_to_pattern(f):
    ...
    if isinstance(f, WinExecutableFile):
        windows_executable_file_expression = convert_windows_executable_file_to_pattern(
            f
        )
...
_REGISTRY_KEY_VALUES_PROPERTIES = [
...
def convert_registry_key_to_pattern(reg_key):
    ...
    if reg_key.key or reg_key.hive:
        key_value_term = ""
        if reg_key.hive:
            if reg_key.hive.condition is None or is_equal_condition(
                reg_key.hive.condition
            ):
                key_value_term += reg_key.hive.value + "\\"
            else:
                warn(
                    "Condition %s on a hive property not handled",
                    812,
                    reg_key.hive.condition,
                )
    ...
    if reg_key.values:
        ...
        for v in reg_key.values:
            ...
            if value_expressions:
                values_expressions.append(
                    create_boolean_expression("OR", value_expressions)
                )
...
def convert_image_info_to_pattern(image_info):
    ...
    if image_info.command_line:
        expressions.append(
            add_comparison_expression(image_info.command_line, "process:command_line")
        )
    if image_info.current_directory:
        expressions.append(
            add_comparison_expression(image_info.current_directory, "process:cwd")
        )
    if expressions:
        return create_boolean_expression("AND", expressions)
...
def convert_process_to_pattern(process):
    ...
    if hasattr(process, "argument_list") and process.argument_list:
        ...
        if get_option_value("spec_version") == "2.0":
            ...
            if argument_expressions:
                expressions.append(
                    create_boolean_expression("AND", argument_expressions)
                )
        else:
            warn(
                "The argument_list property of ProcessObj is not part of STIX 2.1", 418
            )
            lhs = generate_lhs_for_missing_property(
                "process:", None, "argument_list[*]", "process"
            )
            if lhs:
                for a in process.argument_list:
                    argument_expressions.append(
                        create_term(lhs, a.condition, stix2.StringConstant(a.value))
                    )
                if argument_expressions:
                    expressions.append(
                        create_boolean_expression("AND", argument_expressions)
                ...
                if not check_for_missing_policy("ignore"):
                    expressions.append(
                        UnconvertedTerm("ProcessObj.argument_list", "process")
                    )
    if (
        hasattr(process, "environment_variable_list")
        and process.environment_variable_list
    ):
        ev_expressions = []
        for ev in process.environment_variable_list:
            # TODO: handle variable names with '-'
            ev_expressions.append(
                create_term(
                    "process:environment_variables[*]." + str(ev.name),
                    ev.value.condition,
    ...
    if hasattr(process, "network_connection_list") and process.network_connection_list:
        ...
        if network_connection_expressions:
            expressions.append(
                create_boolean_expression("AND", network_connection_expressions)
            )
...
_WINDOWS_SERVICE_PROPERTIES = [
    ...
    [
        ...
        "process:extensions.'windows-service-ext'.startup_command_line",
    ],
...
def convert_windows_service_to_pattern(service):
    ...
    if hasattr(service, "description_list") and service.description_list:
        ...
        for d in service.description_list:
            description_expressions.append(
                create_term(
                    "process:extensions.'windows-service-ext'.descriptions[*]",
                    d.condition,
        ...
        if description_expressions:
            expressions.append(
                create_boolean_expression("AND", description_expressions)
            )
    if hasattr(service, "service_dll") and service.service_dll:
        # assuming its not a path
        expressions.append(
            create_term(
                "process:" + "service_dll_refs[*].name",
...
def convert_domain_name_to_pattern(domain_name, related_objects):
    pattern = [
        create_term(
            "domain-name:value",
            domain_name.value.condition,
            make_constant(domain_name.value.value),
        )
    ]
    if related_objects:
            ...
            if ro.relationship == "Resolved_To":
                ...
                if new_pattern:
                    if isinstance(new_pattern, IdrefPlaceHolder):
                        pattern.append(
                            ComparisonExpressionForElevator(
                                "=", "domain-name:resolves_to_refs[*]", new_pattern
                            )
                        )
                    else:
                        pattern.append(
                ...
                warn(
                    "The %s relationship involving %s is not explicitly supported in STIX 2.x",
                    427,
                    ro.relationship,
                    identifying_info(ro),
                )
...
def convert_host_name_to_pattern(host_name, related_objects):
    pattern = [
        create_term(
            "domain-name:value",
            host_name.hostname_value.condition,
            make_constant(host_name.hostname_value.value),
        )
    ]
    if related_objects:
        for ro in related_objects:
...
def convert_mutex_to_pattern(mutex):
    if mutex.name:
        return create_term(
            "mutex:name", mutex.name.condition, make_constant(mutex.name.value)
        )
    else:
        return None
    ...
    if prop.port_value:
        ...
        expressions.append(
            create_term(
                ...
                prop.port_value.condition,
                make_constant(prop.port_value.value),
            )
        )
        ...
        if prop.layer4_protocol.value is not None:
            expressions.append(
                create_term(
                    ...
                    prop.layer4_protocol.condition,
...
def convert_socket_address_to_pattern(sock_add, direction):
    ...
    if sock_add.port is not None:
        if sock_add.port.port_value is not None:
            expressions.append(
                create_term(
                    "network-traffic:" + direction + "_port",
    ...
    if sock_add.ip_address is not None:
        if sock_add.ip_address.address_value:
            expressions.append(
                create_term(
                    "network-traffic:" + direction + "_ref.value",
            ...
            new_pattern = handle_object_reference_for_pattern(
                sock_add.ip_address.object_reference
            )
            if isinstance(new_pattern, IdrefPlaceHolder):
                ...
                expressions.append(
                    new_pattern.collapse_reference(
                        ObjectPathForElevator.make_object_path(
                            "network-traffic:" + direction + "_ref"
                        )
                    )
                )
    if sock_add.hostname is not None:
        if sock_add.ip_address is not None:
            warn(
                "Only one of the properties: Hostname and IP_Address is allowed.  Dropping Hostname %s",
                645,
                sock_add.hostname.hostname_value,
            ),
        elif (
            sock_add.hostname.is_domain_name
            and sock_add.hostname.hostname_value is not None
        ):
            expressions.append(
                create_term(
                    "network-traffic:" + direction + "_ref.value",
        ...
        elif sock_add.hostname.naming_system is not None and any(
            x.value == "DNS" for x in sock_add.hostname.naming_system
        ):
            expressions.append(
                create_term(
                    "network-traffic:" + direction + "_ref.value",
...
def convert_network_connection_to_pattern(conn):
    ...
    if conn.source_socket_address is not None:
        expressions.extend(
            convert_socket_address_to_pattern(conn.source_socket_address, "src")
        )

    if conn.destination_socket_address is not None:
        expressions.extend(
            convert_socket_address_to_pattern(conn.destination_socket_address, "dst")
        )

    if conn.layer7_connections is not None:
        if conn.layer7_connections.http_session is not None:
            extension_expressions = convert_http_session_to_pattern(
                conn.layer7_connections.http_session
            )
...
def convert_http_client_request_to_pattern(http_request):
    ...
    if http_request.http_request_line is not None:
        if http_request.http_request_line.http_method is not None:
            term = add_comparison_expression(
                http_request.http_request_line.http_method,
        ...
        if http_request.http_request_line.value is not None:
            term = add_comparison_expression(
                http_request.http_request_line.value,
    ...
    if http_request.http_message_body is not None:
        ...
        if mb.message_body:
            expressions.append(
                create_term(
                    "network-traffic:extensions.'http-request-ext'.message_body_data_ref.payload_bin",
                    "Equals",
...
_NETWORK_CONNECTION_PROPERTIES = [
    ...
    [
        "accept_charset",
        "network-traffic:extensions.'http-request-ext'.request_header.'Accept-Charset'",
    ],
    [
        "accept_language",
        "network-traffic:extensions.'http-request-ext'.request_header.'Accept-Language'",
    ],
    [
        "accept_datetime",
        "network-traffic:extensions.'http-request-ext'.request_header.'Accept-Datetime'",
    ],
    [
        "accept_encoding",
        "network-traffic:extensions.'http-request-ext'.request_header.'Accept-Encoding'",
    ],
    [
        "authorization",
        "network-traffic:extensions.'http-request-ext'.request_header.Authorization",
    ],
    [
        "cache_control",
        "network-traffic:extensions.'http-request-ext'.request_header.'Cache-Control'",
    ],
    [
        "connection",
        "network-traffic:extensions.'http-request-ext'.request_header.Connection",
    ],
    ["cookie", "network-traffic:extensions.'http-request-ext'.request_header.Cookie"],
    [
        "content_length",
        "network-traffic:extensions.'http-request-ext'.request_header.'Content-Length'",
    ],
    [
        "content_md5",
        "network-traffic:extensions.'http-request-ext'.request_header.'Content-MD5'",
    ],
    [
        "content_type",
        "network-traffic:extensions.'http-request-ext'.request_header.'Content-Type'",
    ],
    ...
    [
        "if_match",
        "network-traffic:extensions.'http-request-ext'.request_header.'If-Match'",
    ],
    [
        "if_modified_since",
        "network-traffic:extensions.'http-request-ext'.request_header.'If-Modified-Since'",
    ],
    [
        "if_none_match",
        "network-traffic:extensions.'http-request-ext'.request_header.'If-None-Match'",
    ],
    [
        "if_range",
        "network-traffic:extensions.'http-request-ext'.request_header.'If-Range'",
    ],
    [
        "if_unmodified_since",
        "network-traffic:extensions.'http-request-ext'.request_header.'If-Unmodified-Since'",
    ],
    [
        "max_forwards",
        "network-traffic:extensions.'http-request-ext'.request_header.'Max-Forwards'",
    ],
    ["pragma", "network-traffic:extensions.'http-request-ext'.request_header.Pragma"],
    [
        "proxy_authorization",
        "network-traffic:extensions.'http-request-ext'.request_header.'Proxy-Authorization'",
    ],
    ...
    [
        "x_requested_with",
        "network-traffic:extensions.'http-request-ext'.request_header.'X-Requested-With'",
    ],
    [
        "x_forwarded_for",
        "network-traffic:extensions.'http-request-ext'.request_header.'X-Forwarded-For'",
    ],
    [
        "x_att_deviceid",
        "network-traffic:extensions.'http-request-ext'.request_header.'X-ATT-DeviceId'",
    ],
    [
        "x_wap_profile",
        "network-traffic:extensions.'http-request-ext'.request_header.'X-Wap-Profile'",
    ],
]


def generate_lhs_for_missing_property(
    prefix, predefined_extension_name, property_name, object_type
):
    if check_for_missing_policy("use-custom-properties"):
        ...
        if predefined_extension_name:
            return (
                prefix
                + "extensions."
                + predefined_extension_name
                + "."
                + convert_to_custom_name(property_name)
            )
        else:
        ...
        if extension_definition_id:
            ...
            warn(
                "No extension-definition was found for STIX 1 type %s", 312, object_type
            )
            return None
    else:
...
def handle_missing_properties_in_expression_for_icmp_header(expressions, icmp_header):
    if icmp_header.checksum:
        lhs = generate_lhs_for_missing_property(
            "network-traffic:", "icmp-ext", "icmp_checksum", "icmp-header"
        )
        if lhs:
            expressions.append(
                create_term(
    ...
    if packet.internet_layer:
        ...
        if internet_layer.ipv4 or internet_layer.ipv6:
            ...
            if icmp_header.type_:
                expressions.append(
                    create_term(
                        "network-traffic:extensions.'icmp-ext'.icmp_type_hex",
            ...
            if icmp_header.code:
                expressions.append(
                    create_term(
                        "network-traffic:extensions.'icmp-ext'.icmp_type_code",
                        icmp_header.code.condition,
                        stix2.HexConstant(str(icmp_header.code)),
                    )
                )
            handle_missing_properties_in_expression_for_icmp_header(
                expressions, icmp_header
            )
            return create_boolean_expression("AND", expressions)
...
def convert_http_session_to_pattern(session):
    if session.http_request_response:
        requests, responses = split_into_requests_and_responses(
            session.http_request_response
        )
        if len(responses) != 0:
        ...
        if len(requests) >= 1:
            ...
            if len(requests) > 1:
                warn(
                    "Only HTTP_Request_Response used for http-request-ext, using first value",
                    512,
                )
            return expression
...
def convert_socket_options_to_pattern(options):
    ...
    for prop_name in SOCKET_OPTIONS:
        ...
        if value:
            expressions.append(
                create_term(
                    "network-traffic:extensions.'socket-ext'.options."
                    + prop_name.upper(),
                    "Equals",
                    value,
                )
...
def handle_missing_properties_in_expression_for_network_socket(expressions, socket):
    if socket.local_address:
        lhs = generate_lhs_for_missing_property(
            "network-traffic:", "socket-ext", "local_address", "network-socket"
        )
        if lhs:
            expressions.append(
                create_term(
    ...
    if socket.remote_address:
        lhs = generate_lhs_for_missing_property(
            "network-traffic:", "socket-ext", "remote_address", "network-socket"
        )
        if lhs:
            expressions.append(
                create_term(
                    ...
                    stix2.StringConstant(
                        socket.remote_address.ip_address.address_value.value
                    ),
                )
            )
...
def convert_network_socket_to_pattern(socket):
    ...
    if socket.address_family:
        if socket.address_family in ADDRESS_FAMILY_ENUMERATION:
            ...
            warn(
                "%s in is not a member of the %s enumeration",
                627,
                socket.address_family,
                "address family",
            )
    ...
    if socket.protocol:
        expressions.append(
            add_comparison_expression(socket.protocol, "network-traffic:protocols[*]")
        )
...
def convert_product_to_pattern(prod):
    ...
    if prod.language:
        expressions.append(
            add_comparison_expression(prod.language, "software:languages[*]")
        )
    if expressions:
        return create_boolean_expression("AND", expressions)
...
_X509_V3_PROPERTY_MAP = [
    ...
    [
        ...
        "x509-certificate:x509_v3_extensions.subject_key_identifier",
    ],
    [
        "authority_key_identifier",
        "x509-certificate:x509_v3_extensions.authority_key_identifier",
    ],
    [
        "subject_alternative_name",
        "x509-certificate:x509_v3_extensions.subject_alternative_name",
    ],
    [
        "issuer_alternative_name",
        "x509-certificate:x509_v3_extensions.issuer_alternative_name",
    ],
    [
        "subject_directory_attributes",
        "x509-certificate:x509_v3_extensions.subject_directory_attributes",
    ],
    [
        "crl_distribution_points",
        "x509-certificate:x509_v3_extensions.crl_distribution_points",
    ],
    ["inhibit_any_policy", "x509-certificate:x509_v3_extensions.inhibit_any_policy"],
    [
        "certificate_policies",
        "x509-certificate:x509_v3_extensions.certificate_policies",
    ],
...
def convert_v3_extension_to_pattern(v3_ext):
    ...
    if v3_ext.private_key_usage_period:
        if v3_ext.private_key_usage_period.not_before:
            expressions.append(
                add_comparison_expression(
                    v3_ext.private_key_usage_period.not_before,
        ...
        if v3_ext.private_key_usage_period.not_after:
            expressions.append(
                add_comparison_expression(
                    v3_ext.private_key_usage_period.not_after,
    ...
    if x509.certificate:
        ...
        if cert.validity:
            if cert.validity.not_before:
                expressions.append(
                    add_comparison_expression(
                        cert.validity.not_before, "x509-certificate:validity_not_before"
                    )
                )
            if cert.validity.not_after:
                expressions.append(
                    add_comparison_expression(
                        cert.validity.not_after, "x509-certificate:validity_not_after"
                    )
                )
        if cert.subject_public_key:
            if cert.subject_public_key.public_key_algorithm:
                add_comparison_expression(
                    cert.subject_public_key.public_key_algorithm,
            ...
            if cert.subject_public_key.rsa_public_key:
                ...
                if rsa_key.modulus:
                    add_comparison_expression(
                        rsa_key.modulus, "x509-certificate:subject_public_key_modulus"
                    )
                if rsa_key.exponent:
                    add_comparison_expression(
                        rsa_key.exponent, "x509-certificate:subject_public_key_exponent"
                    )
...
def convert_object_to_pattern(obj, obs_id):
    ...
    if prop:
        if isinstance(prop, Address):
            ...
            expression = convert_socket_address_to_pattern(
                prop, determine_socket_address_direction(prop, obs_id)
            )
            if expression:
                expression = create_boolean_expression("AND", expression)
        elif isinstance(prop, Custom):
            is_custom_object = True
            if check_for_missing_policy(
                "use-custom-properties"
            ) or check_for_missing_policy("use-extensions"):
                if prop.custom_name:
                    if check_for_missing_policy("use-custom-properties"):
                        object_path_root = convert_to_custom_name(
                            prop.custom_name, separator="-"
                        )
                    else:  # check_for_missing_policy("use-extensions")
                        if re.search("[A-Z]", prop.custom_name):
                            warn(
                                "Custom name %s has been converted to all lower case",
                                727,
                                prop.custom_name,
                            )
                        object_path_root = prop.custom_name.lower()
                    term = convert_custom_properties(
                        prop.custom_properties,
                        object_path_root,
                        use_custom_prefix=False,
                    )
                    if expression:
                        expression = create_boolean_expression(
                            "AND", [expression, term]
                        )
                    else:
                        expression = term
                ...
                warn(
                    "Pattern expression with STIX 1.x custom objects in %s is ignored",
                    817,
                    obs_id,
                )
        else:
            warn(
                "%s found in %s cannot be converted to a pattern, yet.",
                808,
                str(obj.properties),
                obs_id,
            )
        ...
        if prop.custom_properties is not None and not is_custom_object:
            if check_for_missing_policy(
                ...
                object_type_name = convert_cybox_class_name_to_object_path_root_name(
                    prop
                )
                if check_for_missing_policy("use-custom-properties"):
                    object_path_root = object_type_name
                else:  # check_for_missing_policy("use-extensions")
                    extension_definition_id = get_extension_definition_id(
                        object_type_name
                    )
                    if extension_definition_id:
                        object_path_root = (
                            object_type_name + "extensions." + extension_definition_id
                        )
                        warn(
                            "Used %s for extension property for %s",
                            317,
                            object_path_root,
                            object_type_name,
                        )
                        ...
                        warn(
                            "No extension-definition was found for STIX 1 type %s",
                            312,
                            object_type_name,
                        )
                if object_path_root:
                    term = convert_custom_properties(
                        prop.custom_properties, object_path_root
                    )
                    if expression:
                        expression = create_boolean_expression(
                            "AND", [expression, term]
                ...
                warn(
                    "Pattern expression with STIX 1.x custom properties in %s is ignored",
                    818,
                    obs_id,
                )
    if not expression:
    ...
    for o in obj.related_objects:
        ...
        if not o.relationship == "Resolved_To":
            ...
            if o.related_objects:
                next_level_of_relationships = convert_related_objects_to_pattern(
                    o, obs_id
                )
    ...
    if obs.observable_composition is not None:
        ...
        if obs.object_.idref is not None:
            ...
            if obs.object_.related_objects:
                related_patterns = convert_related_objects_to_pattern(
                    obs.object_, obs.id_
                )
                if pattern:
                    if related_patterns:
        ...
        for fr_idref in fully_resolved_idrefs:
            for idref, expr in get_items_from_pattern_cache():
                if expr and expr.contains_placeholder():
                    change_made, expr = expr.replace_placeholder_with_idref_pattern(
                        fr_idref
                    )
...
def convert_indicator_to_pattern_without_negate(ind):
    if ind.composite_indicator_expression is not None:
        pattern = convert_indicator_composition_to_pattern(
            ind.composite_indicator_expression
        )
...
def remove_pattern_objects(bundle_instance):
    if not KEEP_OBSERVABLE_DATA_USED_IN_PATTERNS:
        ...
        for obj in bundle_instance["objects"]:
            if (
                obj["type"] != "observed-data"
                or obj["id"] not in all_new_ids_with_patterns
                and not exists_id_of_obs_in_characterizations(obj["id"])
            ):
                remaining_objects.append(obj)
            elif exists_id_of_obs_in_characterizations(obj["id"]):
                warn(
                    "%s is used as a characteristic in an infrastructure object, therefore it is not included as an observed_data instance",
                    419,
                    obj["id"],
                )
                ...
                warn(
                    "%s is used as a pattern, therefore it is not included as an observed_data instance",
                    423,
                    obj["id"],
                )
        ...
        for obj in bundle_instance["objects"]:
            if obj["type"] == "sighting" and "observed_data_refs" in obj:
                for ref in obj["observed_data_refs"]:
                    if (
                        exists_id_of_obs_in_sightings(ref)
                        and ref in all_new_ids_with_patterns
                    ):
                        warn(
                            "Observable object from pattern cannot be an observed_data_ref of a sighting. See %s",
                            644,
                            obj["id"],
                        )
        ...
        for obj in remaining_objects:
            if (
                ...
                and "source_ref" in obj
                and "target_ref" in obj
            ):
                if (
                    obj["source_ref"] not in obj_ids_to_delete
                    and obj["target_ref"] not in obj_ids_to_delete
                ):
                ...
                if "object_refs" in obj:
                    for ident in obj["object_refs"]:
                        if not ident.startswith("observed-data") or (
                            ident not in all_new_ids_with_patterns
                            and ident not in obj_ids_to_delete
                        ):
