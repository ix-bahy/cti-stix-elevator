...
from stix2elevator.convert_pattern import (
    ...
    convert_observable_list_to_pattern,
...
def process_kill_chain(kc):
    ...
    for kcp in kc.kill_chain_phases:
        ...
        if check_lmco(kcp):
            ...
            if kcp.phase_id:
                _KILL_CHAINS_PHASES[kcp.phase_id] = {
                    "kill_chain_name": "lockheed-martin-cyber-kill-chain",
                    "phase_name": kcp_name,
                }
            else:
                _KILL_CHAINS_PHASES[kcp] = {
                    "kill_chain_name": "lockheed-martin-cyber-kill-chain",
                    "phase_name": kcp_name,
                }
        else:
...
def get_identity(identity, env, created_by_ref_source, temp_marking_id=None):
    if not env.get_identity_called:
        ...
        ident_instance = convert_identity(
            identity, new_env, created_by_ref_source, temp_marking_id=temp_marking_id
        )
...
def get_identity_ref(identity, env, created_by_ref_source, temp_marking_id=None):
    if identity.idref is not None:
        ...
        ident20 = convert_identity(
            identity, env, created_by_ref_source, temp_marking_id=temp_marking_id
        )
...
def handle_missing_properties_of_information_source(so, information_source):
    ...
    container, extension_definition_id = determine_container_for_missing_properties(
    ...
    if container is not None:
        ...
        if information_source.tools:
            for tool in information_source.tools:
                warn(
                    "Missing property 'tool' %s is ignored",
                    307,
                    ("of" + so["id"] if "id" in so else ""),
                )
    ...
    if information_source:
        if information_source.identity is not None:
            if information_source.identity.idref:
                ...
                identity_ref = get_identity(
                    information_source.identity, env, "this_identity", temp_marking_id
                )
        ...
        if so == env.bundle_instance:
            warn(
                "Information Source on %s is not representable in STIX 2.x",
                401,
                so["id"],
            )
        else:
            if identity_instance:
                if information_source.description:
                    process_description_and_short_description(
                        identity_instance, information_source
                    )
                if information_source.references:
                    for ref in information_source.references:
                        identity_instance["external_references"].append(
                            {"source_name": "unknown", "url": ref}
                        )
                if (
                    information_source.roles
                    and get_option_value("spec_version") == "2.1"
                ):
                    identity_instance["roles"] = [
                        str(r) for r in information_source.roles
                    ]
                handle_missing_properties_of_information_source(
                    identity_instance, information_source
                )
        if information_source.contributing_sources:
            info(
                "Information Source property Contributing_Sources in %s is not handled, yet.",
                815,
                so["id"],
            )
        if information_source.time:
            info(
                "Information Source property Time in %s is not handled, yet.",
                815,
                so["id"],
            )
    else:
...
def process_short_description(so, short_property, short_property_name, parent_info):
    ...
    if short_description_as_text:
        info(
            "The Short_Description property in %s is not supported in STIX 2.x.",
            310,
            so["id"],
        )
        if not check_for_missing_policy("ignore"):
            info(
                "The text in the %s property was appended to the description property of %s",
                301,
                short_property_name,
                so["id"],
            )
            ...
            warn(
                "Missing property '%s' of %s is ignored",
                307,
                short_property_name,
                so["id"],
            )
...
def process_description_and_short_description(so, entity, parent_info=False):
    ...
    if hasattr(entity, "short_descriptions") and len(entity.short_descriptions) != 0:
        ...
        if so["description"]:
            so["description"] += "\n"
        process_short_description(
            so, entity.short_description, "short_description", parent_info
        )
...
def process_description_and_short_description_of_sighting(sighting, indicator_id):
    ...
    if hasattr(sighting, "descriptions") and sighting.descriptions is not None:
        description_as_text = str(process_structured_text_list(sighting.descriptions))
        if description_as_text:
            description_for_sighting += description_as_text + "\n"

    # could be short_description or description (in STIX 1.1.1)
    # seems like in STIX 2.x - description and descriptionS are both populated with the same content
    elif hasattr(sighting, "description") and sighting.description is not None:
        description_for_sighting += str(sighting.description.value)
    if (
        hasattr(sighting, "short_description")
        and sighting.short_description is not None
    ):
        short_description_as_text = str(sighting.short_description)
        if short_description_as_text:
            info(
                "The Short_Description property in %s is not supported in STIX 2.x.",
                310,
                "in a sighting of " + indicator_id,
            )
            if not check_for_missing_policy("ignore"):
                info(
                    "The text was appended to the description property of %s",
                    301,
                    "in a sighting of " + indicator_id,
                )
                description_for_sighting += short_description_as_text
            else:
                warn(
                    "Missing property 'short_description' of %s is ignored",
                    307,
                    "in a sighting of " + indicator_id,
                )
...
def create_basic_object(
    stix2x_type, stix1x_obj, env, parent_id=None, id_used=False, prime_properties=True
):
    instance = {"type": stix2x_type}
    if get_option_value("spec_version") == "2.1":
        instance["spec_version"] = "2.1"
    instance["id"] = generate_stix2x_id(
        stix2x_type,
        (
            stix1x_obj.id_
            if (stix1x_obj and hasattr(stix1x_obj, "id_") and stix1x_obj.id_)
            else parent_id
        ),
        id_used,
    )
...
def convert_marking_specification(
    marking_specification, env, stix1x_id, isa_marking, isa_marking_assertions
):
    return_obj = []
    ...
    if marking_specification.marking_structures is not None:
        ...
        for marking_structure in marking_structures:
            ...
            if (
                marking_structure.idref
                or marking_structure.__class__.__name__ == "MarkingStructure"
            ):
                if not check_map_1x_markings_to_2x(marking_structure):
                    # Don't print message multiple times if idref has been resolved.
                    warn(
                        "Could not resolve Marking Structure. Skipped object %s",
                        425,
                        identifying_info(marking_structure),
                    )
                # Skip empty markings or ones that use the idref approach.
                continue

            if not check_map_1x_markings_to_2x(marking_structure):
                marking_definition_instance = create_basic_object(
                    "marking-definition", marking_structure, env
                )
                process_information_source(
                    marking_specification.information_source,
            ...
            if isinstance(marking_structure, TLPMarkingStructure):
                ...
                if marking_structure.is_proprietary is not None:
                    consent_marking = str(
                        marking_structure.is_proprietary.ais_consent.consent
                    ).lower()
                    if consent_marking != "everyone":
                        warn(
                            "cisa-proprietary is only permitted when ais-consent is everyone, so it has been dropped. See %s",
                            637,
                            stix1x_id,
                        )
                    ...
                    if (
                        marking_structure.is_proprietary.ais_consent is not None
                        and marking_structure.is_proprietary.ais_consent.consent
                        is not None
                    ):
                        definition["consent"] = consent_marking
                        consent_label = "ais-consent-" + definition["consent"]
                    if (
                        marking_structure.is_proprietary.tlp_marking is not None
                        and marking_structure.is_proprietary.tlp_marking.color
                        is not None
                    ):
                        definition["tlp"] = str(
                            marking_structure.is_proprietary.tlp_marking.color
                        ).lower()
                        set_tlp_reference(
                            marking_definition_instance,
                            definition["tlp"],
                    ...
                    consent_marking = str(
                        marking_structure.not_proprietary.ais_consent.consent
                    ).lower()
                    definition["is_proprietary"] = "false"
                    if (
                        marking_structure.not_proprietary.ais_consent is not None
                        and marking_structure.not_proprietary.ais_consent.consent
                        is not None
                    ):
                        definition["consent"] = consent_marking
                        consent_label = "ais-consent-" + definition["consent"]
                        add_label(marking_definition_instance, consent_label)
                    if (
                        marking_structure.not_proprietary.tlp_marking is not None
                        and marking_structure.not_proprietary.tlp_marking.color
                        is not None
                    ):
                        definition["tlp"] = str(
                            marking_structure.not_proprietary.tlp_marking.color
                        ).lower()
                        set_tlp_reference(
                            marking_definition_instance,
                            definition["tlp"],
                ...
                if marking_structure.__class__.__name__ in get_option_value(
                    "markings_allowed"
                ):
                    warn(
                        "Could not resolve Marking Structure %s",
                        425,
                        identifying_info(marking_structure),
                    )
                else:
            ...
            if "definition_type" in marking_definition_instance:
                val = add_marking_map_entry(
                    marking_structure, marking_definition_instance
                )
                if val is not None and not isinstance(val, MarkingStructure):
                    info(
                        "Found same marking structure %s, using %s",
                        625,
                        identifying_info(marking_specification),
                        val,
                    )
                else:
                    info(
                        "Created Marking Structure for %s",
                        212,
                        identifying_info(marking_structure),
                    )
                    finish_basic_object(
                        marking_specification.id_,
                        marking_definition_instance,
                        env,
                        marking_structure,
                    )
                    ...
                    if get_option_value("acs"):
                        for m in isa_marking_assertions:
                            if not check_map_1x_markings_to_2x(m):
                                marking_definition_instance = create_basic_object(
                                    "marking-definition", m, env
                                )
                                convert_edh_marking_to_acs_marking(
                                    marking_definition_instance, isa_marking, m
                                )
                                ...
                                warn(
                                    "Used extensions for ACS data markings. See %s",
                                    319,
                        ...
                        if get_option_value("spec_version") == "2.1":
                            warn(
                                "ACS data markings only supported when --acs option is used. See %s",
                                436,
                                isa_marking.identifier,
                            )
                        else:
                            warn(
                                "ACS data markings cannot be supported in version 2.0.",
                                217,
                            )
...
def create_marking_union(*stix1_objects):
    ...
    for stix1_object in stix1_objects:
        stix2_marking_refs = get_object_marking_refs(
            get_marking_specifications(stix1_object)
        )
...
def finish_markings(instance, env, marking_specifications, temp_marking_id=None):
    ...
    for marking_specification in marking_specifications:
        stix2x_markings, ignore = convert_marking_specification(
            ...
            if not check_map_1x_markings_to_2x(marking_structure):
                for m in stix2x_markings:
                    if m.get("definition_type", "") == "ais":
                        ...
                        if (
                            instance["id"] != m["id"]
                            and m["id"] not in object_marking_refs
                        ):
                ...
                stix2x_marking = map_1x_markings_to_2x(marking_structure)
                if (
                    instance["id"] != stix2x_marking["id"]
                    and stix2x_marking["id"] not in object_marking_refs
                ):
                    if (
                        "definition_type" in stix2x_marking
                        and stix2x_marking["definition_type"] == "ais"
                    ):
...
def finish_basic_object(old_id, instance, env, stix1x_obj, temp_marking_id=None):
    ...
    if (
        stix1x_obj
        and hasattr(stix1x_obj, "related_packages")
        and stix1x_obj.related_packages is not None
    ):
        for p in stix1x_obj.related_packages:
            warn(
                "Related_Packages type in %s not supported in STIX 2.x",
                402,
                stix1x_obj.id_,
            )
...
def handle_sightings_observables(related_observables, env):
    ...
    for ref in related_observables:
        if ref.item.idref is None:
            ...
            for new20 in new20s:
                add_id_of_obs_in_sightings(new20["id"])
                refs.append(new20["id"])
        else:
            add_id_of_obs_in_sightings(ref.item.idref)
        ...
        if information_source.identity is not None:
            return [
                get_identity_ref(
                    ...
                    env,
                    created_by_ref_source="this_identity",
                )
            ]
            # if information_source.description:
...
def handle_sighting(
    ...
    if sighting:
        if sighting.source:
            sighting_instance["where_sighted_refs"] = (
                process_information_source_for_sighting(sighting.source, env)
            )
        if sighting.related_observables:
            sighting_instance["observed_data_refs"] = handle_sightings_observables(
                sighting.related_observables, env
            )
        add_confidence_to_object(sighting_instance, sighting.confidence)
        sighting_instance["description"] = (
            process_description_and_short_description_of_sighting(
                sighting, sighted_object_id
            )
        )
    else:
        if first_seen:
        ...
        if description:
            sighting_instance["description"] = description
        if source_refs:
...
def create_relationship(
    source_ref, target_ref, env, verb, rel_obj=None, marking_refs=None
):
    ...
    if (
        rel_obj is not None
        and hasattr(rel_obj, "relationship")
        and rel_obj.relationship is not None
    ):
...
def handle_embedded_ref(
    stix1_relationship, item, ref1, env, default_verb, to_direction, marking_refs
):
    new_stix2_instances = handle_embedded_object(item, env)
    for new_2x in new_stix2_instances:
        if to_direction:
...
def handle_existing_ref(
    stix1_relationship, ref1, ref2, env, default_verb, to_direction, marking_refs
):
...
def handle_relationship_ref(
    ref, item, id_, env, default_verb, to_direction=True, marking_refs=None
):
    if item.idref is None:
        handle_embedded_ref(
            ref, item, id_, env, default_verb, to_direction, marking_refs
        )
        ...
        rel_obj = create_relationship(
            source_id, target_id, env, default_verb, item, marking_refs
        )
...
def handle_relationship_to_refs(refs, source_id, env, default_verb, marking_refs=None):
    for ref in refs:
        ...
        handle_relationship_ref(
            ...
            default_verb,
            to_direction=True,
            marking_refs=refs_markings,
        )


def handle_relationship_from_refs(
    refs, target_id, env, default_verb, marking_refs=None
):
    for ref in refs:
        if hasattr(ref, "item"):
            item = ref.item
    ...
    for o in obs_list:
        ...
        if o.idref is None and o.object_ and not o.object_.idref:
            ...
            for obj_ref in new_od["object_refs"]:
                env.bundle_instance["relationships"].append(
                    create_relationship(
                        source_id, obj_ref, env, verb, marking_refs=obs_markings
                    )
                )
        else:
            if o.idref:
            ...
            if id_in_observed_data_mappings(idref):
                ...
                for ref in obs2x["object_refs"]:
                    env.bundle_instance["relationships"].append(
                        create_relationship(
                            source_id, ref, env, verb, marking_refs=obs_markings
                        )
                    )
            else:
                if id_in_observable_mappings(idref):
                    # handling a reference, scos generated later
                    new_od = convert_observed_data(
                        get_obs_from_mapping(idref), env, keep_scos=False
                    )
                    add_id_of_obs_in_characterizations(new_od["id"])
                    ...
                    env.bundle_instance["relationships"].append(
                        create_relationship(
                            source_id, idref, env, verb, marking_refs=obs_markings
                        )
                    )
    ...
    for ref in bundle_instance["relationships"]:
        if is_stix1x_id(ref["source_ref"]):
            if not exists_id_key(ref["source_ref"]):
                ...
                if new_id is None:
                    error(
                        "Dangling source reference %s in %s",
                        601,
                        ref["source_ref"],
                        ref["id"],
                    )
                ...
                if first_one:
                    ...
                    extra_relationships.append(
                        create_relationship(
                            ...
                            env,
                            ref["verb"],
                            marking_refs=ref.get("object_marking_refs", []),
                        )
                    )
        if is_stix1x_id(ref["target_ref"]):
            if not exists_id_key(ref["target_ref"]):
                ...
                if new_id is None:
                    error(
                        "Dangling target reference %s in %s",
                        602,
                        ref["target_ref"],
                        ref["id"],
                    )
        ...
        for rel in bundle_instance["relationships"]:
            if ("source_ref" in rel and rel["source_ref"] in refs_in_this_report) and (
                ...
                if (
                    ...
                    and rel["target_ref"]
                    and (
                        rel["target_ref"] in new_ids
                        or exists_ids_with_no_1x_object(rel["target_ref"])
                    )
                ):
                    rels_to_include.append(rel["id"])
                    rels_to_include.append(rel["target_ref"])
                    info(
                        "Including %s in %s and added the target_ref %s to the report",
                        704,
                        rel["id"],
                        rep["id"],
                        rel["target_ref"],
                    ...
                    rels_to_include.append(rel["id"])
                    warn(
                        "Including %s in %s although the target_ref is unknown",
                        706,
                        rel["id"],
                        rep["id"],
                    )
                    ...
                    warn(
                        "Not including %s in %s because there is no corresponding SDO for %s",
                        708,
                        rel["id"],
                        rep["id"],
                        rel["target_ref"],
                ...
                if (
                    ...
                    and rel["source_ref"]
                    and (
                        rel["source_ref"] in new_ids
                        or exists_ids_with_no_1x_object(rel["source_ref"])
                    )
                ):
                    rels_to_include.append(rel["id"])
                    rels_to_include.append(rel["source_ref"])
                    info(
                        "Including %s in %s and added the source_ref %s to the report",
                        705,
                        rel["id"],
                        rep["id"],
                        rel["source_ref"],
                    ...
                    rels_to_include.append(rel["id"])
                    warn(
                        "Including %s in %s although the source_ref is unknown",
                        707,
                        rel["id"],
                        rep["id"],
                    )
...
def handle_missing_required_property(property_name, instance):
    if get_option_value("ignore_required_properties"):
        warn(
            "Required property '%s' is not populated on %s",
            437,
            property_name,
            instance["id"],
        )
        ...
        instance[property_name] = (
            "Placeholder for " + property_name + " property in " + instance["id"]
        )
        warn(
            "A placeholder was generated for required property '%s' of %s",
            438,
            property_name,
            instance["id"],
        )
...
def handle_missing_properties_of_campaign(campaign_instance, camp):
    ...
    if container is not None:
        ...
        handle_missing_string_property(
            ...
            campaign_instance["id"],
            is_literal=True,
            mapping=CAMPAIGN_STATUS_MAP,
        )

        if get_option_value("spec_version") == "2.0":
            handle_missing_confidence_property(
                container, camp.confidence, campaign_instance["id"]
            )
        else:  # 2.1
            add_confidence_to_object(campaign_instance, camp.confidence)

        fill_in_extension_properties(
            campaign_instance, container, extension_definition_id
        )
...
def convert_campaign(camp, env):
    ...
    if "created_by_ref" in campaign_instance:
        new_env = env.newEnv(
            timestamp=campaign_instance["created"],
            created_by_ref=campaign_instance["created_by_ref"],
        )
    else:
        new_env = env.newEnv(timestamp=campaign_instance["created"])
    # process information source before any relationships
    new_env.add_to_env(
        created_by_ref=process_information_source(
            camp.information_source, campaign_instance, new_env
        )
    )
    ...
    if camp.activity is not None:
        for a in camp.activity:
            warn(
                "Campaign/Activity type in %s not supported in STIX 2.x",
                403,
                campaign_instance["id"],
            )
    ...
    if camp.related_incidents is not None and get_option_value("incidents"):
        handle_relationship_from_refs(
            camp.related_incidents,
            campaign_instance["id"],
            new_env,
            "attributed-to",
            marking_refs=camp_markings,
    ...
    if camp.associated_campaigns:
        info(
            "All 'associated campaigns' relationships of %s are assumed to not represent STIX 1.2 versioning",
            710,
            camp.id_,
        )
        handle_relationship_to_refs(
    ...
    if objective is not None:
        if check_for_missing_policy("ignore"):
            ...
            if objective.applicability_confidence:
                handle_missing_confidence_property(
                    container, objective.applicability_confidence, id, "objective"
                )


def handle_missing_properties_of_course_of_action(coa_instance, coa):
    container, extension_definition_id = determine_container_for_missing_properties(
        "course-of-action", coa_instance
    )

    if container is not None:
        handle_missing_string_property(
            container, "stage", coa.stage, coa_instance["id"], is_literal=True
        )
        handle_missing_objective_property(container, coa.objective, coa_instance["id"])
        if coa.parameter_observables is not None:
            parameter_expression = handle_observable_information_list_as_pattern(
        ...
        handle_missing_statement_properties(
            container, coa.impact, "impact", coa_instance["id"], is_literal=True
        )
        handle_missing_statement_properties(
            container, coa.cost, "cost", coa_instance["id"], is_literal=True
        )
        handle_missing_statement_properties(
            container, coa.efficacy, "efficacy", coa_instance["id"], is_literal=True
        )
...
def convert_course_of_action(coa, env):
    ...
    if coa.type_:
        convert_controlled_vocabs_to_open_vocabs(
            coa_instance, "labels", [coa.type_], COA_LABEL_MAP, False
        )
    if coa.structured_coa:
        warn(
            "Structured COAs type in %s are not supported in STIX 2.x",
            404,
            coa_instance["id"],
        )
    ...
    new_env.add_to_env(
        created_by_ref=process_information_source(
            coa.information_source, coa_instance, new_env
        )
    )
    # process information source before any relationships
    if coa.related_coas:
        ...
        info(
            "All 'associated coas' relationships of %s are assumed to not represent STIX 1.2 versioning",
            710,
            coa.id_,
        )
        handle_relationship_to_refs(
...
def create_required_name_from_external_references(
    sdo_instance, type_of_obj, usable_source_names
):
...
def process_et_properties(sdo_instance, et, env):
    ...
    if "name" in sdo_instance:
        info(
            "Title in %s used for name, appending exploit_target %s title in description property",
            303,
            sdo_instance["type"],
            sdo_instance["id"],
        )
        ...
        sdo_instance["name"] = create_required_name_from_external_references(
            sdo_instance, "vulnerability", ["cve", "osvdb"]
        )
    new_env = env.newEnv(timestamp=sdo_instance["created"])
    new_env.add_to_env(
        created_by_ref=process_information_source(
            et.information_source, sdo_instance, new_env
        )
    )
...
def handle_missing_properties_of_vulnerability(vulnerability_instance, v):
    container, extension_definition_id = determine_container_for_missing_properties(
        "vulnerability", vulnerability_instance
    )

    if container is not None:
        if v.source is not None:
            handle_missing_string_property(
                container, "source", v.source, vulnerability_instance["id"], False
            )

        if v.cvss_score is not None:
            # FIXME: add CVSS score into description
            info(
                "CVSS Score in %s is not handled, yet.",
                815,
        ...
        if v.affected_software is not None:
            info(
                "Affected Software in %s is not handled, yet.",
                815,
                vulnerability_instance["id"],
            )

        fill_in_extension_properties(
            vulnerability_instance, container, extension_definition_id
        )


def convert_vulnerability(v, et, env, used_id):
    vulnerability_instance = create_basic_object(
        "vulnerability", v, env, et.id_, used_id
    )
    ...
    if v.cve_id is not None:
        vulnerability_instance["external_references"].append(
            {"source_name": "cve", "external_id": v.cve_id}
        )
    if v.osvdb_id is not None:
        vulnerability_instance["external_references"].append(
            {"source_name": "osvdb", "external_id": str(v.osvdb_id)}
        )
    ...
    if v.references is not None:
        for ref in v.references:
            vulnerability_instance["external_references"].append(
                {"source_name": "internet_resource", "url": ref}
            )
...
def convert_exploit_target(et, env):
    ...
    if et.weaknesses is not None:
        for w in et.weaknesses:
            warn(
                "ExploitTarget/Weaknesses type in %s not supported in STIX 2.x",
                405,
                et.id_,
            )
    if et.configuration is not None:
        for c in et.configuration:
            warn(
                "ExploitTarget/Configurations type in %s not supported in STIX 2.x",
                406,
                et.id_,
            )
    ...
    if party_name.person_names:
        ...
        for name in party_name.person_names:
            if first_one:
                ...
                warn(
                    "Only one person name allowed for %s in STIX 2.x, used %s, %s becomes an alias",
                    502,
                    obj["id"],
                    obj["name"],
                    alias_name,
        ...
        for name in party_name.organisation_names:
            if first_one:
                ...
                aliases.append(alias_name)
                warn(
                    "Only one organization name allowed for %s in STIX 2.x, used %s, %s becomes an alias",
                    503,
                    obj["id"],
                    obj["name"],
                    alias_name,
...
def convert_ciq_addresses2_1(
    ciq_info_addresses, identity_instance, env, created_by_ref_source, parent_markings
):
    location_keys = []
    for ciq_info_address in ciq_info_addresses:
        if not ciq_info_address.free_text_address:
            ...
            if (
                hasattr(ciq_info_address, "administrative_area")
                and ciq_info_address.administrative_area
                and hasattr(ciq_info_address, "country")
                and ciq_info_address.country
            ):
                if len(ciq_info_address.country.name_elements) == 1:
                    country_code = determine_country_code(
                        ciq_info_address.country.name_elements[0]
                    )
                    for (
                        administrative_area
                    ) in ciq_info_address.administrative_area.name_elements:
                        location_keys.append(
                            "c:"
                            + str(country_code)
                            + ","
                            + "aa:"
                            + str(determine_administrative_area(administrative_area))
                        )
                else:
                    warn(
                        "Multiple administrative areas with multiple countries in %s is not handled",
                        631,
                        None,
                    )
            elif (
                ...
                for (
                    administrative_area
                ) in ciq_info_address.adminstrative_area.name_elements:
                    location_keys.append(
                ...
                for country_code in ciq_info_address.country.name_elements:
                    location_keys.append(
                        "c:" + str(determine_country_code(country_code))
                    )
        else:
            # only remember locations with no free text address
            warn(
                "Location with free text address in %s not handled yet",
                433,
        ...
        for key in location_keys:
            if exists_location_object(key):
                ...
                warn(
                    "Location %s may not contain all aspects of the STIX 1.x CIQAddress object",
                    803,
                    location["id"],
                )
            ...
            relationship = create_relationship(
                ...
                location["id"],
                (
                    env.newEnv(created_by_ref=identity_instance["id"])
                    if created_by_ref_source == "this_identity"
                    else env
                ),
                "located-at",
            ...
            info(
                "Included parent markings for Relationship %s and Location %s",
                729,
                relationship["id"],
                location["id"],
            )
...
def handle_missing_properties_of_ciq_instance(identity_instance, ciq, spec_version):
    container, extension_definition_id = determine_container_for_missing_properties(
        "identity-ciq", identity_instance
    )
    ...
    if container is not None:
        if ciq.roles:
            if spec_version == "2.0":
                ...
                warn(
                    "Roles is not a property of an identity in 2.0 (%s).  Perhaps the roles are associated with a related Threat Actor",
                    428,
                    identity_instance["id"],
                )
            else:
        ...
        if ciq._specification.free_text_lines:
            ...
            for line in ciq._specification.free_text_lines:
                lines += line.value
            handle_missing_string_property(
                container, "free_text_lines", lines, identity_instance["id"]
            )

        fill_in_extension_properties(
            identity_instance, container, extension_definition_id
        )
...
def convert_identity(
    identity,
    env,
    created_by_ref_source,
    parent_id=None,
    temp_marking_id=None,
    victim=False,
):
    ...
    if isinstance(identity, CIQIdentity3_0Instance):
        handle_missing_properties_of_ciq_instance(
            identity_instance, identity, spec_version
        )
        ...
        if ciq_info.party_name:
            if "name" in identity_instance:
                warn(
                    "CIQ name found in %s, overriding other name",
                    711,
                    identity_instance["id"],
                )
        ...
        if ciq_info.organisation_info:
            ...
            warn(
                "Based on CIQ information, %s is assumed to be an organization",
                716,
                identity_instance["id"],
            )
            if ciq_info.organisation_info.industry_type:
                ...
                convert_controlled_vocabs_to_open_vocabs(
                    identity_instance, "sectors", industry, SECTORS_MAP, False
                )
        if ciq_info.addresses:
            if spec_version == "2.1":
                parent_markings = create_marking_union(identity)
                convert_ciq_addresses2_1(
                    ciq_info.addresses,
                    identity_instance,
                    env,
                    created_by_ref_source,
                ...
                warn(
                    "CIQ Address information in %s is not representable in 2.0",
                    435,
                    identity.id_ if hasattr(identity, "id_") else parent_id,
                )
...
def handle_incident_time_info(container, incident_instance, incident):
    ...
    if time_values.first_malicious_action:
        handle_missing_timestamp_property(
            ...
            time_values.first_malicious_action.value,
    ...
    if time_values.initial_compromise:
        handle_missing_timestamp_property(
            ...
            time_values.initial_compromise.value,
            incident_instance["id"],
        )
    if time_values.first_data_exfiltration:
        handle_missing_timestamp_property(
            ...
            time_values.first_data_exfiltration.value,
            incident_instance["id"],
        )
    if time_values.incident_discovery:
        handle_missing_timestamp_property(
            ...
            time_values.incident_discovery.value,
            incident_instance["id"],
        )
    if time_values.incident_opened:
        handle_missing_timestamp_property(
            ...
            time_values.incident_opened.value,
            incident_instance["id"],
        )
    if time_values.containment_achieved:
        handle_missing_timestamp_property(
            ...
            time_values.containment_achieved.value,
            incident_instance["id"],
        )
    if time_values.restoration_achieved:
        handle_missing_timestamp_property(
            ...
            time_values.restoration_achieved.value,
            incident_instance["id"],
        )
    if time_values.incident_reported:
        handle_missing_timestamp_property(
            ...
            time_values.incident_reported.value,
            incident_instance["id"],
        )
    if time_values.incident_closed:
        handle_missing_timestamp_property(
            ...
            time_values.incident_closed.value,
            incident_instance["id"],
        )


def handle_missing_identity_ref_properties(
    container, instance2x, sources, env, property_name
):
    ...
    if not identities == list():
        handle_missing_string_property(
            container, property_name, identities, instance2x["id"], is_list=True
        )
...
def handle_missing_properties_of_incident(incident_instance, incident, env):
    ...
    if container is not None:
        ...
        if get_option_value("spec_version") == "2.0":
            handle_missing_confidence_property(
                container, incident.confidence, incident_instance["id"]
            )
        else:  # 2.1
            add_confidence_to_object(incident_instance, incident.confidence)

        if incident.contacts is not None:
            handle_missing_identity_ref_properties(
                container, incident_instance, incident.contacts, env, "contact_refs"
            )
        ...
        if incident.reporter is not None:
            ...
            if reporter.identity:
                ...
                env.bundle_instance["objects"].append(id2x)
                handle_missing_string_property(
                    container, "reporter_ref", id2x["id"], incident_instance["id"]
                )

        if incident.responders is not None:
            handle_missing_identity_ref_properties(
                container, incident_instance, incident.responders, env, "responder_refs"
            )
        ...
        if incident.victims is not None:
            handle_missing_identity_ref_properties(
                container, incident_instance, incident.victims, env, "victim_refs"
            )

        if incident.affected_assets is not None:
            # FIXME: add affected_assets to description
            info(
                "Incident Affected Assets in %s is not handled, yet.",
                815,
                incident_instance["id"],
            )
        ...
        if incident.impact_assessment is not None:
            ...
            info(
                "Incident Impact Assessment in %s is not handled, yet",
                815,
                incident_instance["id"],
            )
        ...
        fill_in_extension_properties(
            incident_instance, container, extension_definition_id
        )
...
def convert_incident(incident, env):
    ...
    if incident.categories is not None:
        convert_controlled_vocabs_to_open_vocabs(
            incident_instance, "labels", incident.categories, INCIDENT_LABEL_MAP, False
        )
    # process information source before any relationships
    new_env.add_to_env(
        created_by_ref=process_information_source(
            incident.information_source, incident_instance, new_env
        )
    )
    ...
    if incident.related_incidents:
        info(
            "All 'related incidents' relationships of %s are assumed to not represent STIX 1.2 versioning",
            710,
            incident_instance["id"],
        )
        handle_relationship_to_refs(
...
def convert_kill_chain_missing_names(phase, kill_chain_phases_2x):
    ...
    if check_lmco(phase):
        ...
        kill_chain_phases_2x.append(
            {"kill_chain_name": kill_chain_name, "phase_name": phase_name}
        )
    else:
        kill_chain_phases_2x.append(
            {"kill_chain_name": kill_chain_name, "phase_name": phase_name}
        ...
        for phase in kill_chain_phases:
            if isinstance(phase, KillChainPhaseReference):
                try:
                    ...
                    kill_chain_phases_2x.append(
                        {
                            "kill_chain_name": kill_chain_info["kill_chain_name"],
                            "phase_name": kill_chain_info["phase_name"],
                        }
                    )
                except KeyError:
                    warn(
                        "Unknown phase_id %s in %s",
                        632,
                        phase.phase_id,
                        sdo_instance["id"],
                    )
...
def convert_test_mechanism(indicator, indicator_instance):
    if indicator.test_mechanisms is not None:
        if (
            not _ALLOW_YARA_AND_SNORT_PATTENS
            and get_option_value("spec_version") == "2.0"
        ):
            warn(
                "YARA/SNORT/IOC or other patterns are not supported in STIX 2.0. See %s",
                504,
        ...
        if hasattr(indicator_instance, "pattern"):
            ...
            warn(
                "Only one type pattern can be specified in %s - using 'stix'",
                712,
                indicator_instance["id"],
            )
        else:
            for tm in indicator.test_mechanisms:
                if hasattr(indicator_instance, "pattern"):
                    ...
                    warn(
                        ...
                        506,
                        indicator_instance["id"],
                        indicator_instance["pattern_type"],
                        determine_pattern_type(tm),
                    )
                else:
                    if isinstance(tm, YaraTestMechanism):
                        ...
                        warn(
                            "IOC indicator in %s cannot be converted to a STIX pattern",
                            410,
                            indicator_instance["id"],
                        )
                        indicator_instance["pattern"] = bytes.decode(
                            etree.tostring(tm.ioc)
                        )
    ...
    if not sightings.sightings_count:
        for s in sightings:
            env.bundle_instance["objects"].append(
                handle_sighting(s, indicator_instance["id"], 1, env)
            )
    elif sightings.sightings_count == 1 and len(sightings) == 1:
        env.bundle_instance["objects"].append(
            handle_sighting(
                sightings[0], indicator_instance["id"], sightings.sightings_count, env
            )
        )
    elif sightings.sightings_count != 1 and len(sightings) == 1:
        env.bundle_instance["objects"].append(
        ...
        last_seen = find_last_seen_sighting(sightings)
        source_refs = []
        observable_refs = []
        description = ""
        for s in sightings:
            if s.related_observables:
                observable_refs.extend(
                    handle_sightings_observables(s.related_observables, env)
                )
            if s.source:
                source_refs.extend(
                    process_information_source_for_sighting(s.source, env)
                )
            description += process_description_and_short_description_of_sighting(
                s, indicator_instance["id"]
            )
        if sightings.sightings_count and sightings.sightings_count != len(sightings):
            warn(
                "Number of sightings given is different than sightings_count in %s",
                730,
                indicator_instance["id"],
            )
        env.bundle_instance["objects"].append(
            handle_sighting(
                ...
                (
                    len(sightings)
                    if sightings.sightings_count == 1
                    else sightings.sightings_count
                ),
                env,
                first_seen=(
                    convert_timestamp_to_string(first_seen) if first_seen else None
                ),
...
def handle_missing_properties_of_indicator(indicator_instance, indicator):
    ...
    if container is not None:
        ...
        if get_option_value("spec_version") == "2.0":
            handle_missing_confidence_property(
                container, indicator.confidence, indicator_instance["id"]
            )
        else:  # 2.1
            add_confidence_to_object(indicator_instance, indicator.confidence)

        fill_in_extension_properties(
            indicator_instance, container, extension_definition_id
        )
...
def convert_indicator(indicator, env):
    ...
    if indicator.alternative_id is not None:
        for alt_id in indicator.alternative_id:
            indicator_instance["external_references"].append(
                {"source_name": "alternative_id", "external_id": alt_id}
            )
        ...
        for window in indicator.valid_time_positions:
            if "valid_from" not in indicator_instance:
                if not window.start_time:
                    warn(
                        "No start time for the first valid time interval is available in %s, using current time (other time intervals might be more appropriate)",  # noqa
                        619,
                        indicator_instance["id"],
                    )
                    indicator_instance["valid_from"] = indicator_instance["created"]
                else:
                    indicator_instance["valid_from"] = convert_timestamp_to_string(
                        window.start_time.value
                    )
                if not window.end_time:
                    warn(
                        "No end time for the first valid time interval is available in %s, other time intervals might be more appropriate",
                        619,
                        indicator_instance["id"],
                    )
                else:
                    indicator_instance["valid_until"] = convert_timestamp_to_string(
                        window.end_time.value
                    )
            else:
                warn(
                    "Only one valid time window allowed for %s in STIX 2.x - used first one",
                    507,
                    indicator_instance["id"],
                )
        if "valid_from" not in indicator_instance:
            warn(
                "No valid time position information available in %s, using parent timestamp",
                903,
                indicator_instance["id"],
            )
            indicator_instance["valid_from"] = convert_timestamp_of_stix_object(
                indicator, env.timestamp
            )
    ...
    if indicator.observable is not None:
        ...
        indicator_instance["pattern"] = convert_observable_to_pattern(
    ...
    if indicator.composite_indicator_expression is not None:
        ...
        indicator_instance["pattern"] = create_boolean_expression(
            indicator.composite_indicator_expression.operator, expressions
        )
        add_to_pattern_cache(indicator.id_, indicator_instance["pattern"])
        if get_option_value("spec_version") == "2.1":
            indicator_instance["pattern_type"] = "stix"
    if (
        indicator.observable
        and indicator.composite_indicator_expression
        or indicator.composite_indicator_expression
    ):
        warn(
            "Indicator %s has an observable or indicator composite expression which may not supported \
correctly in STIX 2.x - please check this pattern",
    ...
    if "pattern" not in indicator_instance:
        warn(
            "Indicator %s does not contain the information necessary to generate a pattern",
            638,
            indicator_instance["id"],
        )
    env = env.newEnv(timestamp=indicator_instance["created"])
    indicator_created_by_ref = process_information_source(
        indicator.producer, indicator_instance, env
    )
    ...
    if indicator.suggested_coas is not None:
        ...
        handle_relationship_from_refs(
            ...
            env,
            "investigates",
            marking_refs=indicator_markings,
        )
    ...
    if indicator.related_indicators:
        info(
            "All 'related indicators' relationships of %s are assumed to not represent STIX 1.2 versioning",
            710,
            indicator.id_,
        )
        handle_relationship_to_refs(
...
def create_scos(obs, observed_data_instance, env, keep_scos):
    if not obs.object_ and obs.observable_composition:
        warn(
            "%s contains a observable composition, which implies it not an observation, but a pattern and needs "
        ...
        if obs.object_.related_objects:
            for o in obs.object_.related_objects:
                if not o.idref:
                    ...
                    if related:
                        scos.extend(related)
                        property_name = embedded_property_ref_name(
                            obs.object_.properties, o.relationship
                        )
                        if not property_name:
                            marking_refs = create_marking_union(obs.object_, o)
                            rel_verb = (
                                o.relationship.value.lower()
                            ...
                            if o.relationship and o.relationship.value:
                                set_embedded_ref_property_2_1(
                                    scos[0], related[0], property_name
                                )
        if scos:
            for obj in scos:
...
def create_cyber_observables(obs, observed_data_instance):
    if not obs.object_ and obs.observable_composition:
        ...
        if not observed_data_instance["objects"]:
            warn(
                "%s did not yield any STIX 2.x object",
                417,
                obs.object_.id_ if obs.object_.id_ else obs.id_,
            )
        else:
            if obs.object_.related_objects:
                for o in obs.object_.related_objects:
                    ...
                    if related:
                        for index, obj in related.items():
                            observed_data_instance["objects"][
                                str(int(index) + int(current_largest_id) + 1)
                            ] = obj
                        property_name = embedded_property_ref_name(
                            obs.object_.properties, o.relationship
                        )
                        if property_name and o.relationship and o.relationship.value:
                            set_embedded_ref_property_2_0(
                                observed_data_instance["objects"]["0"],
...
def id_in_observed_data_mappings(id_):
    return find_key_in_dict_case_insensitive(id_, _OBSERVABLE_TO_OBSERVED_DATA_MAPPINGS)
...
def convert_observed_data(obs, env, keep_scos=True):
    ...
    if (
        "objects" not in observed_data_instance
        and "object_refs" not in observed_data_instance
    ):
        return None
    info(
        "'first_observed' and 'last_observed' data not available directly on %s - using timestamp",
        901,
        obs.id_,
    )
    ...
    observed_data_instance["number_observed"] = (
        1 if obs.sighting_count is None else obs.sighting_count
    )
    # TODO: created_by
...
def convert_report(report, env):
    ...
    if report.header:
        header_created_by_ref = process_information_source(
            report.header.information_source, report_instance, new_env
        )
    ...
    info(
        "The published property is required for STIX 2.x Report %s, using the created property",
        720,
        report_instance["id"],
    )
    if report.related_reports is not None:
        ...
        info(
            "Report Related_Reports in %s is not handled, yet.",
            815,
            report_instance["id"],
        )
...
def add_motivations_to_threat_actor(sdo_instance, motivations):
    ...
    if motivations[0].value is not None:
        sdo_instance["primary_motivation"] = map_vocabs_to_label(
            str(motivations[0].value), ATTACK_MOTIVATION_MAP
        )
...
def handle_missing_properties_of_threat_actor(threat_actor_instance, threat_actor):
    container, extension_definition_id = determine_container_for_missing_properties(
        "threat-actor", threat_actor_instance
    )
    if container is not None:
        handle_multiple_missing_statement_properties(
            container,
            threat_actor.planning_and_operational_supports,
            "planning_and_operational_support",
            threat_actor_instance["id"],
            is_literal=True,
        )
        if get_option_value("spec_version") == "2.0":
            handle_missing_confidence_property(
                container, threat_actor.confidence, threat_actor_instance["id"]
            )
        else:  # 2.1
...
def convert_threat_actor(threat_actor, env):
    ...
    new_env.add_to_env(
        created_by_ref=process_information_source(
            threat_actor.information_source, threat_actor_instance, new_env
        )
    )
    ...
    if threat_actor.identity is not None:
        if threat_actor.identity.id_:
            info(
                "Threat Actor identity %s being used as basis of attributed-to relationship",
                701,
                threat_actor.identity.id_,
            )
        ta_id_markings = create_marking_union(threat_actor, threat_actor.identity)
        handle_relationship_to_objs(
    ...
    if threat_actor.title is not None:
        ...
        aliases = convert_party_name(
            threat_actor.identity._specification.party_name,
    ...
    if threat_actor.observed_ttps is not None:
        handle_relationship_to_refs(
            threat_actor.observed_ttps,
            threat_actor_instance["id"],
            new_env,
            "uses",
            marking_refs=ta_markings,
        )
    if threat_actor.associated_campaigns is not None:
        handle_relationship_from_refs(
            threat_actor.associated_campaigns,
            threat_actor_instance["id"],
            new_env,
            "attributed-to",
            marking_refs=ta_markings,
    ...
    if threat_actor.associated_actors:
        info(
            "All 'associated actors' relationships of %s are assumed to not represent STIX 1.2 versioning",
            710,
            threat_actor.id_,
        )
        handle_relationship_to_refs(
            threat_actor.associated_actors,
            threat_actor_instance["id"],
            new_env,
            "related-to",
            marking_refs=ta_markings,
...
def determine_ttp_relationship_type_and_direction(
    source_type, target_type, relationship_name
):
...
def handle_missing_properties_of_ttp(sdo_instance, ttp, type_of_ttp):
    container, extension_definition_id = determine_container_for_missing_properties(
        sdo_instance["type"] if not type_of_ttp else type_of_ttp, sdo_instance
    )
    if container is not None:
        handle_multiple_missing_statement_properties(
            container,
            ttp.intended_effects,
            "intended_effects",
            sdo_instance["id"],
            is_literal=True,
            mapping=_INTENDED_EFFECTS_LITERAL_MAPPING,
        ...
        if hasattr(ttp, "title"):
            ...
            if ttp.title != sdo_instance["name"]:
                handle_missing_string_property(
                    container, "title", ttp.title, sdo_instance["id"], False
                )

        fill_in_extension_properties(sdo_instance, container, extension_definition_id)


def process_ttp_properties(
    sdo_instance, ttp, env, kill_chains_in_sdo=True, marking_refs=None, type_of_ttp=None
):
    ...
    ttp_created_by_ref = process_information_source(
        ttp.information_source, sdo_instance, env
    )
    env.add_to_env(created_by_ref=ttp_created_by_ref)
    if ttp.exploit_targets is not None:
        warn(
            "Exploit targets are part of STIX 1x %s.  Assuming they are related.",
            646,
    ...
    if ttp.related_ttps:
        info(
            "All 'related ttps' relationships of %s are assumed to not represent STIX 1.2 versioning",
            710,
            ttp.id_,
        )
        for rel in ttp.related_ttps:
            ...
            if rel.item.idref is None:
                ...
                verb, to_direction = determine_ttp_relationship_type_and_direction(
                    source_type, target_type, str(rel.relationship)
                )
                refs_markings = list(set(create_marking_union(rel) + marking_refs))
                handle_embedded_ref(
                    rel, rel.item, rel.item.id_, env, verb, to_direction, refs_markings
                )
            else:
                ...
                if stix2x_target_ids != []:
                    for id20 in stix2x_target_ids:
                        ...
                        verb, to_direction = (
                            determine_ttp_relationship_type_and_direction(
                                source_type, target_type, str(rel.relationship)
                            )
                        )
                        refs_markings = list(
                            set(create_marking_union(rel) + marking_refs)
                        )
                        handle_existing_ref(
                            rel,
                            id20,
...
def convert_attack_pattern(ap, ttp, env, ttp_id_used):
    attack_pattern_instance = create_basic_object(
        "attack-pattern", ap, env, ttp.id_, not ttp_id_used
    )
    if ap.capec_id is not None:
        attack_pattern_instance["external_references"] = [
            {"source_name": "capec", "external_id": ap.capec_id}
        ]
    if ap.title is not None:
        ...
        warn(
            ...
            437,
            "name",
            attack_pattern_instance["id"],
        )
    else:
        attack_pattern_instance["name"] = create_required_name_from_external_references(
            attack_pattern_instance, "attack-pattern", ["capec"]
        )
...
def handle_missing_properties_of_malware_instance(sdo_instance, malware1x_instance):
    ...
    if container is not None and not check_for_missing_policy("ignore"):
        ...
        if get_option_value("spec_version") == "2.0":
            if (
                malware1x_instance.names is not None
                and len(malware1x_instance.names) > 1
            ):
                handle_missing_string_property(
                    container,
                    "other_names",
                    malware1x_instance.names[1:],
                    sdo_instance["id"],
                    is_list=True,
                )
...
def convert_malware_instance(mal, ttp, env, ttp_id_used):
    malware_instance_instance = create_basic_object(
        "malware", mal, env, ttp.id_, not ttp_id_used
    )
    if get_option_value("spec_version") == "2.1":
        malware_instance_instance["is_family"] = False
        info(
            "The is_family property of malware instance %s is assumed to be false",
            728,
    ...
    if mal.names is not None:
        for n in mal.names:
            if "name" not in malware_instance_instance:
                ...
                warn(
                    "Only one name for malware is allowed for %s in STIX 2.x - used %s, dropped %s",
                    508,
                    malware_instance_instance["id"],
                ...
                aliases.append(alias_name)
                warn(
                    "Only one name for malware is allowed for %s in STIX 2.1, used %s, %s becomes an alias",
                    502,
                    malware_instance_instance["id"],
                    malware_instance_instance["name"],
                    alias_name,
    ...
    process_ttp_properties(
        malware_instance_instance, ttp, env, marking_refs=mi_markings
    )
...
def convert_behavior(behavior, ttp, env):
    ...
    if behavior.exploits is not None:
        for e in behavior.exploits:
            warn(
                "TTP/Behavior/Exploits/Exploit in %s not supported in STIX 2.x",
                408,
                ttp.id_,
            )
...
def handle_missing_properties_of_tool(tool_instance, tool):
    ...
    if container is not None:
        handle_missing_string_property(
            container, "vendor", tool.vendor, tool_instance["id"]
        )
        handle_missing_string_property(
            container, "service_pack", tool.service_pack, tool_instance["id"]
        )
        ...
        if tool.tool_hashes is not None:
            ...
            info(
                "Tool Tool_Hashes in %s is not handled, yet.", 815, tool_instance["id"]
            )
...
def convert_infrastructure(infra, ttp, env, first_one):
    infrastructure_instance = create_basic_object(
        "infrastructure", infra, env, parent_id=ttp.id_, id_used=not first_one
    )
    ...
    info(
        "No 'first_seen' data on %s - using timestamp",
        904,
        infra.id_ if infra.id_ else ttp.id_,
    )
    infrastructure_instance["first_seen"] = convert_timestamp_of_stix_object(
        infra, infrastructure_instance["created"]
    )
    infra_markings = create_marking_union(infra)
    if infra.observable_characterization is not None:
        handle_observable_information_list(
            infra.observable_characterization,
            infrastructure_instance["id"],
            env,
            "consists-of",
            infra_markings,
        )
    process_ttp_properties(
        infrastructure_instance, ttp, env, marking_refs=infra_markings
    )
...
def convert_resources(resources, ttp, env, generated_ttps):
    ...
    if resources.infrastructure is not None:
        if (
            get_option_value("infrastructure")
            or get_option_value("spec_version") == "2.1"
        ):
            new_obj = convert_infrastructure(
                resources.infrastructure, ttp, env, first_one
            )
            env.bundle_instance["objects"].append(new_obj)
            resources_generated.append(new_obj)
        else:
...
def convert_identity_for_victim_target(victim_targeting, ttp, env, ttp_generated):
    if victim_targeting.identity:
        ...
        identity_instance = convert_identity(
            victim_targeting.identity,
            env,
    ...
    if container is not None:
        if victim_targeting.targeted_systems:
            handle_missing_string_property(
                container,
                "targeted_systems",
                victim_targeting.targeted_systems,
                identity_instance["id"],
                True,
            )
        if victim_targeting.targeted_information:
            handle_missing_string_property(
                ...
                victim_targeting.targeted_information,
                identity_instance["id"],
                True,
            )
        # TODO: technical_details are Observables
        if (
            hasattr(victim_targeting, "technical_details")
            and victim_targeting.targeted_technical_details is not None
        ):
            warn(
                "The technical_details property of %s is not part of STIX 2.x",
                418,
                identity_instance["id"],
            )
...
def convert_victim_targeting(victim_targeting, ttp, env, ttps_generated):
    identity_instance = convert_identity_for_victim_target(
        victim_targeting, ttp, env, ttps_generated
    )
...
def convert_ttp(ttp, env):
    if hasattr(ttp, "timestamp") and ttp.timestamp:
        new_env = env.newEnv(
            timestamp=convert_timestamp_of_stix_object(ttp, env.timestamp, True)
        )
    else:
        new_env = env
    generated_objs = []
    ...
    if ttp.resources is not None:
        generated_objs.extend(
            convert_resources(ttp.resources, ttp, new_env, generated_objs)
        )
    ...
    if ttp.victim_targeting is not None:
        victim_target = convert_victim_targeting(
            ttp.victim_targeting, ttp, new_env, generated_objs
        )
        if not victim_target:
            warn(
                "Victim Target in %s did not generate any STIX 2.x object", 414, ttp.id_
            )
        else:
...
def handle_embedded_object(obj, env):
    ...
    if new20:
        ...
        warn(
            "No STIX 2.x object generated from embedded object %s",
            416,
            identifying_info(obj),
        )
        return []
                ...
                for kcp in ind20["kill_chain_phases"]:
                    if isinstance(kcp, str):
                        ...
                        try:
                            ...
                            error(
                                "Dangling kill chain phase id in indicator %s",
                                607,
                                ind20["id"],
                            )
                    else:
            ...
            if "pattern" in ind:
                ...
                if final_pattern:
                    if final_pattern.contains_placeholder():
                        error(
                            "At least one PLACEHOLDER idref was not resolved in %s",
                            205,
                            ind["id"],
                        )
                    if final_pattern.contains_unconverted_term():
                        error(
                            "At least one observable could not be converted in %s",
                            206,
                            ind["id"],
                        )
                    if isinstance(
                        final_pattern, ComparisonExpressionForElevator
                    ) or isinstance(final_pattern, UnconvertedTerm):
                        ind["pattern"] = "[%s]" % final_pattern
                    elif isinstance(final_pattern, ParentheticalExpressionForElevator):
                        result = (
                            final_pattern.expression.partition_according_to_object_path()
                        )
                        if (
                            isinstance(result, BooleanExpressionForElevator)
                            and result.operator == "OR"
                            and result.any_operand_contains_observed_expressions()
                        ):
                            result = maybe_split_parenthetical_ors_into_compound_observation_expressions(
                                result
                            )
                        ...
                        ind["pattern"] = str(
                            final_pattern.partition_according_to_object_path()
                        )
...
def get_identity_from_information_source(
    information_source, env, created_by_ref_source
):
    if information_source:
        if information_source.identity is not None:
            return get_identity_ref(
                information_source.identity, env, created_by_ref_source
            )
            ...
            if information_source.contributing_sources.source is not None:
                ...
                for source in sources:
                    if source.identity is not None:
                        return get_identity_ref(
                            source.identity, env, created_by_ref_source
                        )
    return None


def handle_missing_properties_of_stix_header(
    sdo_instance, stix_package_header, stix_type
):
    container, extension_definition_id = determine_container_for_missing_properties(
        stix_type, sdo_instance
    )
    if container is not None:
...
def create_object_for_package_header(stix_package_header, env, type_of_obj):
    ...
    if type_of_obj == "report":
        ...
        handle_missing_properties_of_stix_header(
            sdo_instance, stix_package_header, type_of_obj
        )
    sdo_instance["object_refs"] = [x["id"] for x in env.bundle_instance["objects"]]
    if type_of_obj == "report":
        sdo_instance["published"] = strftime_with_appropriate_fractional_seconds(
            datetime.now(), True
        )
    if "description" in sdo_instance and sdo_instance["description"] == "":
        del sdo_instance["description"]
    info(
        "%s was created to store the extra STIX package header information",
        220,
        sdo_instance["id"],
    )
    return sdo_instance
...
def stix_header_contains_extra_information(stix_header):
    return stix_header and (
        (hasattr(stix_header, "title") and stix_header.title is not None)
        or (
            hasattr(stix_header, "package_intents")
            and stix_header.package_intents is not None
            and len(stix_header.package_intents) != 0
        )
        or (hasattr(stix_header, "descriptions") and len(stix_header.descriptions) != 0)
        or (hasattr(stix_header, "description") and stix_header.description is not None)
        or (
            hasattr(stix_header, "short_descriptions")
            and len(stix_header.short_descriptions) != 0
        )
        or (
            hasattr(stix_header, "short_description")
            and stix_header.short_description is not None
        )
    )
...
def convert_package(stix_package, env):
    ...
    if hasattr(stix_package, "timestamp") and stix_package.timestamp:
        ...
        env.timestamp = datetime.strptime(
            get_option_value("default_timestamp"), "%Y-%m-%dT%H:%M:%S.%fZ"
        )
    else:
        # timestamp not given on the command line
        env.timestamp = strftime_with_appropriate_fractional_seconds(
            datetime.now(), True
        )
    ...
    if not env.created_by_ref and hasattr(
        stix_package.stix_header, "information_source"
    ):
        env.created_by_ref = get_identity_from_information_source(
            stix_package.stix_header.information_source, env, "this_identity"
        )
    ...
    for marking_specification in navigator.iterwalk(stix_package):
        if isinstance(marking_specification, MarkingSpecification):
            stix2x_markings, isa_marking = convert_marking_specification(
                marking_specification,
                env,
                stix_package.id_,
                isa_marking,
            ...
            for marking in stix2x_markings:
                if (
                    "definition_type" in marking and marking["definition_type"] != "ais"
                ) or "extensions" in marking:
    ...
    if not check_for_missing_policy("ignore"):
        if stix_header_contains_extra_information(stix_package.stix_header):
            ...
            bundle_instance["objects"].append(
                create_object_for_package_header(
                    stix_package.stix_header, env, header_object_type
                )
            )
        else:
            info(
                "The STIX package header contained no extra information that needed to be generated in STIX 2.x",
                219,
            )
    else:
